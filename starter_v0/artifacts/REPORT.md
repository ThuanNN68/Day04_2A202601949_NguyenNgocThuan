...
| ------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- | ------------- | -----: | ----: | ---------------------------------------------------- |
| v0      | baseline                                                                                                                                                                                                                                                    | Baseline initial setup                                                                                                    | case_accuracy |   0.00 |  0.90 | `runs/v0_B_base_openai_20260729T104010144368.json` |
| v1      | Thêm Section 6 Guardrails vào`system_prompt.md`; siết rule Parallel Execution (chỉ gọi cả 2 khi user nói rõ cả 2 nguồn); thêm Query keyword rule (extract keyword ngắn, bỏ filler); mô tả `tools.yaml` chi tiết hơn với routing hints | Siết rule sẽ fix R03 (model tự gọi thêm`social_search`) và R13 (query bị dịch sang tiếng Việt) → đạt 100%  | case_accuracy |   0.90 |  1.00 | `runs/v1_B_base_openai_20260729T122405629969.json` |
| v2      | Tối ưu hoá xử lý đổi nguồn thông tin (Source Switching) và làm sạch context thô                                                                                                                                                                | Kiểm chứng tính ổn định của hệ thống với nhiều test case phức tạp khác (phát hiện lỗi thừa tool ở R20) | case_accuracy |   1.00 |  0.95 | `runs/v2_B_base_openai_20260729T124652257529.json` |
| v3      | Bổ sung quy tắc**Source Switching** chi tiết vào rule 5 của `system_prompt.md` để drop hoàn toàn tool của nguồn cũ khi user đổi ý                                                                                                    | Sẽ khắc phục triệt để lỗi thừa tool R20 khi chuyển từ Twitter sang Web → đạt lại 100%                       | case_accuracy |   0.95 |  1.00 | `runs/v3_B_base_openai_20260729T125130073083.json` |
